...
class InventoryPage:
    def __init__(self, Manage_window):
        ...
        # NAME AND ENTRY
        partsLabel = Label(coverFrame3, text="BILL NUMBER", bg='#ffffff', font=("yu gothic ui", 12, "bold"))
        partsLabel.place(x=90, y=10)
        ...
        partsName_entry.place(x=10, y=40, width=275, height=34)
        ...
        def delete_invoice():
            ...
            try:
                ...
                if ask is True:
                    ...
                    cur.execute(delete, [partsName_entry.get()])
                    conn.commit()
                    show_all()
                    clear_all()
                    conn.close()
        ...
        parts_tree.heading("BillNumber", text="BILL NUMBER", anchor=N)
        parts_tree.heading("Date", text="DATE", anchor=N)
        parts_tree.heading("CashierName", text="CASHIER NAME", anchor=N)
        parts_tree.heading("contact", text="CONTACT", anchor=N)
        #parts_tree.heading("BillDetails", text="#", anchor=N)

        parts_tree.column("#0", stretch=NO, minwidth=0, width=0)
        parts_tree.column("#1", stretch=NO, minwidth=0, width=230, anchor=N)
        parts_tree.column("#2", stretch=NO, minwidth=0, width=150, anchor=N)
        parts_tree.column("#3", stretch=NO, minwidth=0, width=352, anchor=N)
        parts_tree.column("#4", stretch=NO, minwidth=0, width=110, anchor=N)
        #parts_tree.column("#5", stretch=NO, minwidth=0, width=50, anchor=N)
