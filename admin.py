...
class SecondPage:
    def __init__(self, manage_page):
        ...
        manage_page.title("Management System")
        ...
        with sqlite3.connect("./Database/AutoShop.db") as db:
            cur = db.cursor()
        ...
        class Manage:
            def __init__(self, Manage_window):
                ...
                Manage_window.title("Management System")
                ...
                # Manage Parts Label
                manage = Label(coverFrame2, text='MANAGE', font=("yu gothic ui", 13, "bold"), bg='#ffffff',
                                     fg='#ff6c38')
                manage.place(x=80, y=0)

                coverFrame3 = LabelFrame(coverFrame2, bg='#ffffff', bd='2.4')
                coverFrame3.place(x=20, y=80, width=250, height=475)

                # Image
                ShopImage = Image.open('images\\menu1.png')
                photo = ImageTk.PhotoImage(ShopImage)
                ShopImg = Label(coverFrame2, image=photo, bg='#ffffff')
                ShopImg.image = photo
                ShopImg.place(x=100, y=35)
                ...
                idName_entry = Entry(coverFrame3, highlightthickness=2, relief=FLAT, bg="#ffffff", fg="#6b6a69",
                                         font=("", 11, 'bold'), textvariable=parts_id)
                idName_entry.place(x=22, y=44, width=40, height=24)
                idName_entry.config(highlightbackground="#6b6a69", highlightcolor="#ff6c38")

                # PARTS NAME AND ENTRY
                partsLabel = Label(coverFrame3, text="PARTS", bg='#ffffff', font=("yu gothic ui", 12, "bold"))
                partsLabel.place(x=90, y=40)

                partsName_entry = Entry(coverFrame3, highlightthickness=2, relief=FLAT, bg="#ffffff", fg="#6b6a69",
                                         font=("", 12, 'bold'), textvariable=parts)
                partsName_entry.place(x=10, y=70, width=225, height=34)
                partsName_entry.config(highlightbackground="#6b6a69", highlightcolor="#ff6c38")

                # PARTS TYPE AND ENTRY
                ...
                # =====================================================================================================================
                def show_all():
                    conn = sqlite3.connect("./Database/AutoShop.db")
                    cur = conn.cursor()
                    cur.execute("select * from Parts_Category")
                    rows = cur.fetchall()
                    if len(rows) != 0:
                ...
                def parts_info(ev):
                    viewInfo = parts_tree.focus()
                    parts_data = parts_tree.item(viewInfo)
                    row = parts_data['values']
                    parts_id.set(row[0])
                    parts.set(row[1])
                    type.set(row[2])
                    discount.set(row[3])
                ...
                def add_parts():
                    if parts_id.get() == "":
                        messagebox.showerror("Failed", "Parts Number( # ) can't be empty")
                    else:
                        conn = sqlite3.connect("./Database/AutoShop.db")
                        cur = conn.cursor()
                        cur.execute("INSERT INTO Parts_Category values(?,?,?,?,?,?)",
                                    (parts_id.get(), parts.get(), type.get(), discount.get(), in_stock.get(), price.get()))
                        conn.commit()
                        conn.close()
                        show_all()
                        messagebox.showinfo("Success", "Parts Records Added Successfully")

                def delete_records():
                    try:
                        tree_view_content = parts_tree.focus()
                        tree_view_items = parts_tree.item(tree_view_content)
                        ...
                        if ask is True:
                            conn = sqlite3.connect("./Database/AutoShop.db")
                            cur = conn.cursor()
                            cur.execute("DELETE FROM Parts_Category where parts_id=?", [parts_id.get()])
                            conn.commit()
                            show_all()
                            clear_all()
                            conn.close()
                ...
                def update():
                    ...
                    cur = conn.cursor()
                    cur.execute(
                        "UPDATE Parts_Category set parts_name=?,type=?,discount=?,in_stock=?,parts_price=? where "
                        "parts_id=?",
                        (parts.get(), type.get(), discount.get(), in_stock.get(), price.get(), parts_id.get()))
                    conn.commit()
                    conn.close()
                    show_all()
                    # self.Reset()
                    messagebox.showinfo("Success", "Parts Record updated Successfully")
                ...
                self.button4.configure(command=add_parts)
                ...
                parts_tree = ttk.Treeview(coverFrame)
                parts_tree.place(relx=0.0500, rely=0.228, width=896, height=410)
                parts_tree.configure(
                    yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set
                )
                parts_tree.configure(selectmode="extended")

                scrollbary.configure(command=parts_tree.yview)
                scrollbarx.configure(command=parts_tree.xview)
                ...
                parts_tree.heading("PartsID", text="#", anchor=N)
                parts_tree.heading("PartsName", text="PARTS", anchor=N)
                parts_tree.heading("Type", text="TYPE", anchor=N)
                parts_tree.heading("Discount", text="DISCOUNT", anchor=N)
                parts_tree.heading("InStock", text="IN STOCK", anchor=N)
                parts_tree.heading("Price", text="PRICE", anchor=N)

                parts_tree.column("#0", stretch=NO, minwidth=0, width=0)
                parts_tree.column("#1", stretch=NO, minwidth=0, width=50, anchor=N)
                parts_tree.column("#2", stretch=NO, minwidth=0, width=288, anchor=N)
                parts_tree.column("#3", stretch=NO, minwidth=0, width=176, anchor=N)
                parts_tree.column("#4", stretch=NO, minwidth=0, width=110, anchor=N)
                parts_tree.column("#5", stretch=NO, minwidth=0, width=110, anchor=N)
                parts_tree.column("#6", stretch=NO, minwidth=0, width=160, anchor=N)
                parts_tree.bind("<ButtonRelease-1>", parts_info)
                show_all()
        ...
        class PartsItem:
            def __init__(self, parts, price, qty):
                self.product_name = parts
                self.price = price
                self.qty = qty
        ...
        class BillWindow:
            def __init__(self, top=None):
                ...
                self.partsImage = Image.open('images\\menu1.png')
                photo = ImageTk.PhotoImage(self.partsImage)
                self.partsImg = Label(self.coverFrame2, image=photo, bg='#ffffff')
                self.partsImg.image = photo
                self.partsImg.place(x=220, y=30)
                ...
                self.parts_name = Label(biller)
                self.parts_name.place(x=45, y=430)
                self.parts_name.configure(font="-family {Poppins Light} -size 11")
                self.parts_name.configure(foreground="#000000")
                self.parts_name.configure(text="Parts Name")
                self.parts_name.configure(background="#ffffff")
                ...
                find_category = "SELECT type FROM Parts_Category"
                ...
                head = "\n\n\t\t Aliakbar Auto Shop\n" \
                       "\t\t C12 TEMA GHANA\n\n\t\t THANK YOU FOR CHOOSING OUR PARTS\n" \
            ...
            def get_category(self, Event):
                ...
                self.combo2.set('')
                self.combo3.set('')
                find_subcat = "SELECT discount FROM Parts_Category WHERE type = ?"
            ...
            def get_subcat(self, Event):
                ...
                self.combo3.set('')
                find_product = "SELECT parts_name FROM Parts_Category WHERE type = ? and discount = ?"
            ...
            def show_qty(self, Event):
                ...
                find_qty = "SELECT in_stock FROM Parts_Category WHERE parts_name = ?"
            ...
            def add_to_cart(self):
                ...
                if strr.find('Total') == -1:
                    ...
                    if (product_name != ""):
                        ...
                        find_mrp = "SELECT parts_price, in_stock FROM Parts_Category WHERE parts_name = ?"
                        ...
                        if product_qty.isdigit() == True:
                            if (stock - int(product_qty)) >= 0:
                                ...
                                item = PartsItem(product_name, mrp, int(product_qty))
                    ...
                    if (product_name != ""):
                        product_qty = self.entry4.get()
                        find_mrp = "SELECT parts_price, in_stock, parts_id FROM Parts_Category WHERE parts_name = ?"
                        cur.execute(find_mrp, [product_name])
                        results = cur.fetchall()
                        stock = results[0][1]
                        mrp = results[0][0]
                    ...
                    if(cust_name.get()==""):
                        ...
                        if strr.find('Total')==-1:
                            ...
                            for name, qty in self.cart.dictionary.items():
                                update_qty = "UPDATE Parts_Category SET in_stock = in_stock - ? WHERE parts_name = ?"
