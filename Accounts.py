...
class InventoryPage:
    def __init__(self, account_window):
        ...
        account_window.title("Management System")
        ...
        def guest_info(ev):
            ...
            parts_data = self.tree.item(viewInfo)
        ...
        def employee_info(ev):
            ...
            parts_data = employee_tree.item(viewInfo)
            row = parts_data['values']
            employee_id.set(row[0])
            employee_fullname.set(row[1])
            employee_username.set(row[2])
        ...
        #  NAME AND ENTRY
        partsLabel = Label(account_window, text="EMPLOYEE  FULL  NAME", bg='#f6f6f9', font=("yu gothic ui", 12, "bold"))
        partsLabel.place(relx=0.034, rely=0.53)

        partsName_entry = Entry(account_window, highlightthickness=2, relief=FLAT, bg="#ffffff", fg="#6b6a69",
                                 font=("", 12, 'bold'), textvariable=employee_fullname)
        partsName_entry.place(relx=0.019, rely=0.57, width=225, height=34)
        ...
        def admin_info(ev):
            ...
            parts_data = admin_tree.item(viewInfo)
            row = parts_data['values']
            admin_id.set(row[0])
            admin_fullname.set(row[1])
            admin_username.set(row[2])
        ...
        # NAME AND ENTRY
        partsLabel = Label(account_window, text="ADMIN  FULL  NAME", bg='#f6f6f9',
                            font=("yu gothic ui", 12, "bold"))
        partsLabel.place(relx=0.639, rely=0.148)

        partsName_entry = Entry(account_window, highlightthickness=2, relief=FLAT, bg="#ffffff", fg="#6b6a69",
                                 font=("", 12, 'bold'), textvariable=admin_fullname)
        partsName_entry.place(relx=0.619, rely=0.188, width=225, height=34)
        partsName_entry.config(highlightbackground="#6b6a69", highlightcolor="#ff6c38")
